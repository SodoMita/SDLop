...
{
    bool replacing = (keymap != NULL); /* stock: KEYMAP_CHANGED fires for a replaced keymap, not the first one */
    ...
    if (replacing) {
        SDL_Event event;
        SDL_zero(event);
        event.type = SDL_EVENT_KEYMAP_CHANGED; /* app event: common layout */
